...
6. (可选) 不知道 Chrome 版本有没有影响, 可以考虑使用最新版的 Chrome, 更稳妥
...
1. 客户端 (也就是你刚刚下载的 chrome 扩展):
   ...
   2. 接收来自于服务端的指令并执行
      1. 获取 slots, 发送给服务端 (最多 15 分钟获取一次, 取决于在线人数, 人越多间隔时间越长, 被封号的机率越低)
...
根据工作原理来看, 如果有
1. 15 个活跃的客户端, 那么程序会每 1 分钟让一个客户端去获取 slots, 每个客户端 15 分钟获取一次 slots
2. 450 个活跃的客户端, 那么程序会每 2 秒让一个客户端去获取 slots, 每个客户端 15 分钟获取一次 slots
3. 900 个活跃的客户端, 那么程序会每 1 秒让一个客户端去获取 slots, 每个客户端 15 分钟获取一次 slots
